            ...
            <?php
              try {
                ...
                foreach ($lista as $linha) {
                    echo "
                    <div class='modal-header'><img class='capa' width='200' height='300' src='".$linha['img']."'>
...
<div class='modal-body'>  Preço médio<br>
          ...
          <div class="modal-footer">
                <?php
              try {
                $sql = "SELECT * FROM review JOIN user on review.iduser = user.iduser WHERE idproduct=1;";
                $stmt = $conexao->query($sql);
                $lista = $stmt->fetchAll(PDO::FETCH_ASSOC);
                $list = array();
            
                foreach ($lista as $linha) {
                    echo "Nota: ".$linha['score']." <br/>
            Por: ".$linha['name']." <br>
            ".$linha['text']."";
                }
    ...
    try {
      ...
      <div class='produto'>
        <h3>".$linha['title']."</h3>
        <img height='200px' src='".$linha['img']."' alt='Imagem do livro' class='fotolivro'><br/>
